    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex items-center justify-between border-b border-graphite-800 pb-4">
        ...
        <div className="flex items-center gap-2 text-xs font-mono text-graphite-400">
          ...
          <span>/</span>
          <span className="text-cobalt-400 font-bold">#{comic.issue_number}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="rounded-xl border border-graphite-800 bg-graphite-900/90 p-4 shadow-xl">
            ...
            {/* Quick Metadata Spec */}
            <div className="mt-4 pt-4 border-t border-graphite-800 space-y-2 text-xs font-mono">
              ...
              {comic.cover_sha256 && (
                <div className="flex flex-col text-[10px] text-graphite-500 pt-1">
                  ...
                  <span className="truncate font-mono text-graphite-400">{comic.cover_sha256}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="rounded-xl border border-graphite-800 bg-graphite-900/90 p-6 space-y-4 shadow-xl">
            <div className="space-y-2">
              ...
              <h1 className="text-2xl sm:text-4xl font-bold tracking-tight text-chalk">
                {comic.series}{" "}
                <span className="text-cobalt-400 font-mono">#{comic.issue_number}</span>
              </h1>
            ...
            {/* Specification Grid */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-graphite-800 font-mono text-xs">
              <div className="rounded bg-graphite-950 p-2.5 space-y-0.5">
                <span className="text-[10px] uppercase text-graphite-500">VOLUME</span>
                <p className="font-semibold text-chalk">{comic.volume || "1"}</p>
              </div>

              <div className="rounded bg-graphite-950 p-2.5 space-y-0.5">
                <span className="text-[10px] uppercase text-graphite-500">PRINTING</span>
                <p className="font-semibold text-chalk">{comic.printing ? `${comic.printing}st Printing` : "1st Printing"}</p>
              </div>

              <div className="rounded bg-graphite-950 p-2.5 space-y-0.5">
                <span className="text-[10px] uppercase text-graphite-500">PUB DATE</span>
                <p className="font-semibold text-chalk">{formatDate(comic.publication_date)}</p>
              </div>

              <div className="rounded bg-graphite-950 p-2.5 space-y-0.5">
                <span className="text-[10px] uppercase text-graphite-500">UPC CODE</span>
                <p className="font-semibold text-chalk truncate">{comic.upc || "—"}</p>
              </div>
            </div>
          </div>
