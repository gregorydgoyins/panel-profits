...
create index if not exists pp_news_stories_active_idx
  on public.pp_news_stories (published_at desc, ingested_at desc)
  where archived_at is null;

create index if not exists pp_news_stories_archive_idx
  on public.pp_news_stories (archived_at desc);
...
create policy "pp_news_stories_read"
  on public.pp_news_stories for select
  using (true);
...
as $$
  ...
  where archived_at is null
    and coalesce(published_at, ingested_at) < now() - interval '3 days';
$$;
